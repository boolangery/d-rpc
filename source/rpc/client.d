...
@("Test RpcInterfaceClient")
unittest
{
    import std.stdio;
    import vibe.core.stream: OutputStream;
    import vibe.core.core: IOMode;

    interface IAPI { int add(int a, int b); }

    static class TestStream: OutputStream
    {
        private IRpcProcessor _processor;

        void setProcessor(IRpcProcessor processor)
        {
        ...
        {
            _processor.process(`{"jsonrpc":"2.0","id":1,"result":5}`);
            return bytes.length;
        }
    }

    auto stream = new TestStream();
    auto client = new RawJsonRpcClient!int(stream);
    stream.setProcessor(client);

    auto api = new RpcInterfaceClient!(IAPI, RpcProtocol.jsonRpc2_0)(client);
